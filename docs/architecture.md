...
3. Sample timestamps with OpenCV, resize previews, and score intensity/layout/histogram
   differences. Tiny moving areas are suppressed. Compare against both the preceding
   preview and last retained state to catch gradual changes.
...
7. Enrich only retained frames with bounded worker pools. Vision rate limiting also
   applies to retries. Cache successful normalized output, not errors.
8. Join speech with overlapping visual events. Later speech on static screens creates
   its own events referencing preceding evidence, so frame dedupe never deletes speech.
...
usage; frame enrichment is parallel. Sampling uses decoder seeks and low-resolution scoring,
not persistent full-resolution candidate files. Long GOP media can still make seeking costly.
